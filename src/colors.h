...
const static std::set<std::string> color_names{ // from grDevices::colors()
...
inline bool is_color(std::string& name) {
  if (name[0] == '#') {
    if (name.size() != 4 && name.size() != 5 && name.size() != 7 && name.size() != 9) return false;
    if (name.find_first_not_of("0123456789abcdefABCDEF", 1) != std::string::npos) return false;
    ...
    return color_names.find(name) != color_names.end();
  }
  return false;
}
